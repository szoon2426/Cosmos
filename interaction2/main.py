...
def handle_keypress(
    ...
    if key == b"1":
        ...
        toggles["open_spread"] = max(0.0, float(toggles["open_spread"]) - 0.1)
        toggles["open_branch"] = "open"
        ...
        toggles["rise_height"] = max(0.0, float(toggles["rise_height"]) - 0.1)
        toggles["rise_ready_until"] = now + RISE_READY_GRACE
        toggles["rise_release"] = float(toggles["rise_height"]) <= 0.0
        return True, f"rise_height={toggles['rise_height']:.2f}"
    elif key in (b"-", b"_"):
        toggles["rise_height"] = max(0.0, float(toggles["rise_height"]) - 0.1)
        toggles["rise_ready_until"] = now + RISE_READY_GRACE
...
def print_header() -> None:
    ...
    print("A/D or J/L or Left/Right: open spread down/up | P: open end")
    print("W/S or I/K or Up/Down or -/=: rise down/up | Space hold: gather")
