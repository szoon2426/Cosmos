...
class InteractionEngine:
    ...
    def _update_open(self, signals: InteractionSignals, events: EngineEvents) -> None:
        ...
        if state.phase == "active":
            ...
            state.previous_amount = state.amount
            state.amount = clamp(signals.open_spread, 0.0, 1.0)
    ...
    def _update_rise(self, signals: InteractionSignals, events: EngineEvents) -> None:
        ...
        if state.phase == "active":
            state.previous_amount = state.amount
            state.amount = clamp(signals.rise_height, 0.0, 1.0)
    ...
    def _update_emotion(self, dt: float) -> None:
        ...
        open_delta = self.interaction.open.amount - self.interaction.open.previous_amount
        rise_delta = self.interaction.rise.amount - self.interaction.rise.previous_amount
        gather_active = self.interaction.gather.phase == "active"
        breath_active = self.interaction.deep_breath.phase == "active"

        self.emotion.valence += open_delta * 0.45
        self.emotion.dominance += open_delta * 0.35

        self.emotion.arousal += rise_delta * 0.55
        self.emotion.valence += rise_delta * 0.12
        self.emotion.dominance += rise_delta * 0.12
